...
- サーバー→クライアント: `{"visible":true}` / `{"visible":false}`
  ...
  - 上位レイヤーとの重なりによる非表示は `{"visible":false,"reason":"occluded"}`
...
- 自分より優先度が高く実際に表示中のクライアントと矩形が重なる間、
  `{"visible":false,"reason":"occluded"}` で隠される
- 上位のクライアントが切断・非表示になれば再計算で自動的に復帰する
- 同名レイヤーのクライアント同士は後から接続した方が上
- `rect` を申告しないクライアントは調停に参加しない(隠されも隠しもしない)。
  従来のクライアントはそのまま動く
- 全画面クライアント(ssbrowse/fbhalf など)が `rect` を申告すると、上位の
  小さなレイヤー(task-var 等)と重なった時点で全体が隠されてしまう。
  全画面系は rect を申告せず、専用シーンによる排他で制御するのが基本
